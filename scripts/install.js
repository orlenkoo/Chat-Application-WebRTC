...
let output = {
    ...
    REACT_APP_REGISTER_ENABLED: true,
    MAILER_ENABLED: false,
    APP_NAME: 'clover',
    APP_TITLE: 'Clover',
    REACT_APP_SITE_TITLE: 'Clover',
    REACT_APP_PREFER_DARK: true,
    MONGO_URI: 'mongodb://localhost:27017/clover',
    MEDIASOUP_VERBOSE: false,
    POWER_WORKERS: 1,
};
...
(async () => {
    ...
    output['PUBLIC_URL'] = 'https://' + domain;
    ...
    output['ROOT_USER_USERNAME'] = username;
    ...
    output['ROOT_USER_PASSWORD'] = password;
    ...
    output['ROOT_USER_FIRST_NAME'] = firstName;
    ...
    output['ROOT_USER_LAST_NAME'] = lastName;
    ...
    exec(`echo "${outputString}" >> "../.env"`);
    exec('cp ../.env ../frontend/.env');
    exec('cp ../.env ../backend/.env');
    ...
    exec(`echo "module.exports = {aliases: {\\"www.${domain}\\": \\"${domain}\\"}, certs: {default: {type: \\"path\\", cert: \\"certs/127.0.0.1.cert\\", key: \\"certs/127.0.0.1.key\\"}, \\"${domain}\\": {type: \\"letsencrypt\\", email: \\"${email}\\"}}, standard: [80], ssl: [443], servers: {\\"80\\": {\\"${domain}\\": [{forceSSL: true}]}, \\"443\\": {\\"${domain}\\": [{regex: '/api.*', proxy: \\"http://localhost:4011\\", changeOrigin: true, secure: false, ws: true}, {regex: '/socket.*', proxy: \\"http://localhost:4011\\", changeOrigin: true, secure: false, ws: true}, {proxy: \\"http://localhost:4012\\", changeOrigin: true, secure: false, ws: true}]}}};" >> ../acacia/config.js`);
    ...
    try {
        exec('pm2 delete CloverBackend', {stdio : 'pipe'});
    } catch (e) {}
    exec('cd ../backend && pm2 start index.js --name "CloverBackend"');
    ...
    try {
        exec('pm2 delete CloverFrontend', {stdio : 'pipe'});
    } catch (e) {}
    exec('cd ../frontend && pm2 start index.js --name "CloverFrontend"');
